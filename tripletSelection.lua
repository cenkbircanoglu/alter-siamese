...
function triplets(embeddings, numImages, numPerClass, peoplePerBatch, alpha, embSize, cuda)
    local as_table = {}
    local ps_table = {}
    local ns_table = {}
    ...
    local as = torch.concat(as_table):view(table.getn(as_table), embSize)
    local ps = torch.concat(ps_table):view(table.getn(ps_table), embSize)
    local ns = torch.concat(ns_table):view(table.getn(ns_table), embSize)
    ...
    return apn, torch.Tensor(triplet_idx)
end
