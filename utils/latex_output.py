...
if __name__ == '__main__':
    names = glob.glob("/media/cenk/2TB1/alter_siamese/results/mlp_results.txt")
    print(names)
    for name in names:
        print(name)
        ...
        data['dataset'] = data["name"].apply(lambda x: x.split(":")[-1].split("/")[-3])
        ...
        #data["name"] = data['dataset'] + "_" + data["network"] + "_" + data["name"]
        #data.drop("network", axis=1, inplace=True)
        #data.drop("dataset", axis=1, inplace=True)
        data.sort_values(["dataset","network", "name"]).T.to_csv(
            os.path.join(os.path.dirname(os.path.dirname(name)), "mlp_latex.xls"))
