...
def main():
    ...
    parse.add_argument('-n', '--num_images', dest='num_images', type=int)
    ...
    for root, dirs, _ in os.walk(args.folder):
        for dir in dirs:
            files += [os.path.join(root, dir, fn) for fn in os.listdir(os.path.join(root, dir))]
    ...
    res = make_collage(images[:args.num_images], args.output, args.width, args.init_height)
    if not res:
